...
Due to the sheer scale of number of variables that would be branched on under the first approach, we focus on the second approach for branching. Thus, conceptually, we branch in the ambient space of $Z$.
...
\end{algorithmic}
\begin{algorithmic}
\Function{Process}{$\mathcal{S}$, $\mathcal{P}$, $\omega^0$, $\upsilon$, $\left( \widetilde{x}, \widetilde{z} \right)$}
...
% \State $k \gets k+1$
%\EndWhile
\If{$\widecheck{\phi}^{*} > \upsilon$ \textbf{or} $\widecheck{\phi}^* = \infty$}
...
\end{algorithmic}
\begin{algorithmic}[1]
\Function{PSCG}{$\mathcal{S}$, $\mathcal{P}$, $D^k$, $x^k$, $z^k$, $\omega^k$, $\widecheck{\phi}^{k}$ }  
       \For{$k=1,2,\dots,k_{max}$}
       \State Initialize $\omega^{k+1} \gets \omega^{k}$, $\widecheck{\phi}^{k+1} \gets \widecheck{\phi}^{k}$ \Comment{(Default, null-step updates)}
       ...
       \If{$L_\rho({x}^{k+1},{z} ^{k+1},\omega^k) + \frac{\rho}{2} \norm{Q{x}^{k+1}-{z}^{k+1}}_2^2-\widecheck{\phi}^k \le \epsilon$} \label{Alg4BeginTermCond}
       	\State {\bf return} $(x^{k+1},z^{k+1},\omega^{k+1},\widecheck{\phi}^{k+1}, D^{k+1}, \textbf{true})$
...
\medskip
\begin{algorithmic}
\State {\bf Preconditions:} $\widetilde{x} \in \conv(X)$, $\widetilde{z} \in \argmin_z \braces{F(\widetilde{x},z) : z \in Z}$, $D \subseteq \conv(X)$
\end{algorithmic}
\begin{algorithmic}[1]
