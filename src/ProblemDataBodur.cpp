...
void ProblemDataBodur::initialise(PSCGParams *par) {
	//env = iloenv;
	
	if (par->noScenarios > 0)
	{
		readBodur(par);
	}
	else
...
int ProblemDataBodur::readBodur(PSCGParams *par) {
	
	std::string full_filename(par->filename);
	//std::string filepath(request->filepath);
	
	//std::string full_filename = filepath + filename;
	
	int i, j, k;
	p = new double[par->noScenarios];
	nS = par->noScenarios;
	...
	//p = (1.0 / par.noScenarios);
	
	ifstream file(full_filename.c_str());
	
	if (!file) {
		cerr << "ERROR: could not open file '" << full_filename
		<< "' for reading" << endl;
		throw(-1);
	}
